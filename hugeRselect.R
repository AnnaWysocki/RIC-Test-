...
## Main Function
huge.Rselect = function(est, criterion = NULL, ebic.gamma = 0.5, stars.thresh = 0.1, stars.subsample.ratio = NULL, rep.num = 20, verbose = TRUE){
	...
	{		
		...
		if(criterion == "ric")
		{
			if(verbose)
			{
        		...
        		cat("Conducting rotation information criterion (with cool R code)....")
        		flush.console()
        	}
			...
			h$opt.lambda <- RIC(h$data,d,n,r,nr)/n
			
			out = .C("RIC", X = as.double(h$data), dd = as.integer(d), 
